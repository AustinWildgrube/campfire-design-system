...
`SelectDataInterface` can be used for Typescript completion when creating the data to be passed in. The label will appear in the drop-down menu, but the
...
If the data is not in the correct format, the `<usi-option>` component can be used. When the usi-option component is used, it is up to the user to ensure the
correct data is being presented. If the select still needs to be searchable the data will need to be manually filtered. Campfire provides a `usiSearchValue`
output that captures the value entered into the select component.
...
```html
<usi-select (usiSearchValue)="captureValue($event)" usiSearchable>
...
## Disabling Options
Options can be disabled by using a boolean attribute. A disabled option is unusable and unclickable. The disabled attribute can be set to keep a user from
...
## Selects in Forms
Selects will almost always be used in forms. Campfire selects can be used in reactive forms as well as template-driven forms via the methods below.
...
By default, Campfire will display a 'No result' message when the user's search returns a map of size 0. To accommodate other languages, there is a
