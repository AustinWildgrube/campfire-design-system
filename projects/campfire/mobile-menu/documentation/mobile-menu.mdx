...
option for us. For ease of use, consistency, and accessibility, we have created a mobile menu component. The design of a mobile menu is one of the most important
aspects that aims at guiding users in the right direction. Good menu design enhances the user experience of the entire product.
...
available. The `MobileMenuAuth` interface is available to know what information needs to be provided. The authentication object can be provided to the mobile
menu using the square bracket syntax `[usiAuth]`.

> The authentication information can be updated instantaneously using variables, and the mobile menu will update accordingly.
...
### Show
The other required attribute is the `show` attribute. The mobile menu has two different states: `logged` or `guest`. Logged users will see the link when they
...
available. The `icon` attribute can be used along with any type of menu item.
...
### Href
The `href` attribute can be used to redirect the user to a different page. This attribute works the exact same as an `<a>` tag and does not utilize Angular's
built-in router. This will perform a full page refresh when clicked. Because of this, the `href` attribute should be reserved for offsite links. The `href`
cannot be used with the `routerLink` attribute.
...
`selected` attribute determines the default language that is selected. When changing a language it is best to subscribe to the language observable then make the
...
The mobile menu provides a service that can be used to watch and change the state of it. There are five observables: `route`, `isDarkMode`, `isAuthed`,
...
## Customization
The Campfire mobile menu was made to be easily customizable. Styling the mobile menu is as simple as just adding a class onto the tag or overriding our CSS
