...
# Toast Notifications
Toast Notifications inform users of a process that an app has performed or will perform. They appear temporarily,
towards the top right of the screen. They shouldn't interrupt the user experience, and usually don't require user
input to disappear. Toast notification have a default show time of 4 seconds which proves enough time to comprehend
the message and act on it.

Campfire toast notifications are called via a service. The service will first need to be injected into the component on
...
Campfire toast notifications were made to be easily customizable. Styling an inline notification is as simple as just
