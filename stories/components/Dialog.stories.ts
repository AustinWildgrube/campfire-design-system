...
import documentation from 'usi-campfire/dialog/documentation/dialog.mdx';

export default {
  title: 'Feedback/Dialog Modals',
  component: UsiDialogModalComponent,
  decorators: [
    moduleMetadata({
      imports: [UsiModalsModule, UsiButtonModule],
    }),
  ],
  parameters: {
    docs: {
  ...
  argTypes: {
    ...
    usiLimit: {
      name: 'usiLimit',
      description: 'The maximum number of modals that can be displayed at one time.',
      defaultValue: '1',
      type: {
        name: 'number',
        required: false,
...
const Template: Story<UsiDialogModalComponent> = (args: UsiDialogModalComponent) => ({
  props: args,
  template: `
    <usi-dialog-modal [dialog]="dialog"></usi-dialog-modal>
  `,
});
